...
namespace Sabresaurus.SabreCSG
{
	public static class PolygonFactory
	{
	    ...
	    public static Polygon[] GenerateCylinder()
	    {
			int sideCount = 32;
	    ...
	    public static Polygon[] GenerateSphere()
	    {
			int count = 8; // lateral
			int count2 = 16; // longitudinal
...
//	        mesh = new Mesh();
		...
		public static bool IsMeshConvex(Polygon[] polygons)
		{
			...
			for (int i = 0; i < polygons.Length; i++) 
			{
				Plane polygonPlane = new Plane(polygons[i].Vertices[0].Position, polygons[i].Vertices[1].Position, polygons[i].Vertices[2].Position);
				
				for (int j = 0; j < polygons.Length; j++) 
				{
					for (int k = 0; k < polygons[j].Vertices.Length; k++) 
					{
						float dot = Vector3.Dot(polygonPlane.normal, polygons[j].Vertices[k].Position) + polygonPlane.distance;
